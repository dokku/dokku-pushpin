...
@test "($PLUGIN_COMMAND_PREFIX:info) success" {
  ...
  assert_contains "${lines[*]}" "websocket://dokku-pushpin-l:5561"
}
...
@test "($PLUGIN_COMMAND_PREFIX:info) replaces underscores by dash in hostname" {
  ...
  assert_contains "${lines[*]}" "websocket://dokku-pushpin-test-with-underscores:5561"
...
@test "($PLUGIN_COMMAND_PREFIX:info) success with flag" {
  ...
  assert_output "websocket://dokku-pushpin-l:5561"
