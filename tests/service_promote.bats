...
@test "($PLUGIN_COMMAND_PREFIX:promote) changes WEBSOCKET_URL" {
  dokku config:set my_app "WEBSOCKET_URL=websocket://host:5561" "DOKKU_PUSHPIN_BLUE_URL=websocket://dokku-pushpin-l:5561"
...
@test "($PLUGIN_COMMAND_PREFIX:promote) uses PUSHPIN_DATABASE_SCHEME variable" {
  dokku config:set my_app "PUSHPIN_DATABASE_SCHEME=pushpin2" "WEBSOCKET_URL=websocket://host:5561" "DOKKU_PUSHPIN_BLUE_URL=pushpin2://dokku-pushpin-l:5561"
  dokku "$PLUGIN_COMMAND_PREFIX:promote" l my_app
  url=$(dokku config:get my_app WEBSOCKET_URL)
