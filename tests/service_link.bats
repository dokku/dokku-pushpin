...
@test "($PLUGIN_COMMAND_PREFIX:link) exports WEBSOCKET_URL to app" {
  ...
  assert_contains "$url" "websocket://dokku-pushpin-ls:5561"
  assert_success
  dokku "$PLUGIN_COMMAND_PREFIX:unlink" ls my-app
}

@test "($PLUGIN_COMMAND_PREFIX:link) generates an alternate config url when WEBSOCKET_URL already in use" {
  dokku config:set my-app WEBSOCKET_URL=websocket://host:5561
  dokku "$PLUGIN_COMMAND_PREFIX:link" ls my-app
...
@test "($PLUGIN_COMMAND_PREFIX:link) links to app with docker-options" {
  ...
  assert_contains "${lines[*]}" "--link dokku.pushpin.ls:dokku-pushpin-ls"
  assert_success
  dokku "$PLUGIN_COMMAND_PREFIX:unlink" ls my-app
}

@test "($PLUGIN_COMMAND_PREFIX:link) uses apps PUSHPIN_DATABASE_SCHEME variable" {
  dokku config:set my-app PUSHPIN_DATABASE_SCHEME=pushpin2
  dokku "$PLUGIN_COMMAND_PREFIX:link" ls my-app
  url=$(dokku config:get my-app WEBSOCKET_URL)
  assert_contains "$url" "pushpin2://dokku-pushpin-ls:5561"
  assert_success
  dokku "$PLUGIN_COMMAND_PREFIX:unlink" ls my-app
}
