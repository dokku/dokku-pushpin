...
It is possible to change the protocol for `WEBSOCKET_URL` by setting the environment variable `PUSHPIN_DATABASE_SCHEME` on the app. Doing so after linking means unlink no longer finds the variable it set, and leaves it in place, so we advise you to unlink before proceeding.
...
An app is still linked after its `WEBSOCKET_URL` is changed to point elsewhere, and is unlinked the same way. The variable it now holds is not the service's, so it is left alone, nothing is unset, the app is not restarted, and a warning says so.
...
> NOTE: a log setting or a restart policy reaches the container the next time one is built. pushpin:restart keeps the container it has, so use pushpin:stop and then pushpin:start on a service that is already running.
...
> NOTE: a mount reaches the container the next time one is built. pushpin:restart keeps the container it has, so use pushpin:stop and then pushpin:start on a service that is already running.
